...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public void notifyDataSetChanged2() {
		...
		tabCount = title.length;

		for (int i = 0; i < tabCount; i++) {

			addTextTab2(i, title[i]);
	...
	public OnPageSelectListener selectListener;
	
	public void setSelectListener(OnPageSelectListener selectListener) {
		...
		if (pager == null) {
			...
			if (currentTab != null) {
				...
				canvas.drawRect(lineLeft - detalLeft, height - indicatorHeight,
						lineLeft + currentTab.getWidth() - detalRight, height,
						rectPaint);

				Log.i("tteerr", "lineLeft=" + lineLeft + "--" + "lineRight="
						+ lineRight + "--detalLeft=" + detalLeft
						+ "--+detalRight=" + detalRight);
				Log.i("tteerr", "" + "left=" + (lineLeft - detalLeft) + "top="
						+ (height - indicatorHeight) + "right="
						+ (lineLeft + currentTab.getWidth() - detalRight)
						+ "bottom=" + height);

				Log.i("tteerr", "currentPosition=" + currentPosition
	 ...
	 */
	/*
	 * private void clickto(final int fromPosition, final int toPosition) {
	 * 
	 ...
	 * 
	 * 
	 * float lineLeft = currentTab.getLeft(); float lineRight =
	 * currentTab.getRight();
	 * 
	 * float detalLeft = lineLeft - lastTab.getLeft(); float detalRight =
	 * lineRight - lastTab.getRight();
	 * 
	 * if (Math.abs(detalLeft) > minDetal || Math.abs(detalRight) > minDetal) {
	 * 
	 * if (Math.abs(detalLeft) > minDetal) { detalLeft = detalLeft / minDetal; }
	 * 
	 * if (Math.abs(detalRight) > minDetal) { detalRight = detalRight /
	 * minDetal; } invalidate(); PagerSlidingTabStrip.this.post(this); } else {
	 * invalidate(); } } }); }
	 */
